...
class BotsFacet {
	 ...
	 *
	 */
	updateStatus(botId, opts) {
		let args = {
			json : opts
		};
		return this.makeAPICall('POST', `/bots/${botId}/update-status`, args);
	}
	 ...
	 *
	 */
	sendIncomingMessage(botId, opts){
		let args = {
			data : opts
		};
		return this.makeAPICall('POST', `/bots/${botId}/incoming-messages`, args);
	}
	...
	*/
	...
	/**
	 * Send a request to remove a client from a bot
	 *
	 * @param {String} botId Id of the bot to remove a client from
	 * @param {Object=} opts
	 * @param {String} opts.client The Name of the client to remove from the bot
	 *
	 */
	removeClient(botId, opts) {
		let args = {
			data : opts
		};
		return this.makeAPICall('DELETE', `/bots/${botId}/clients`, args);
	}
}
